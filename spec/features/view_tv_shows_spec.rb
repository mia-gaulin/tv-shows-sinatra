...
feature "user views list of TV shows" do
  ...
  scenario "view details for a TV show" do
    ...
    expect(page).to have_content("Network: HBO")
  end
  ...
  scenario "view details for a TV show with missing information" do
    ...
    click_link "Game of Thrones (HBO)"
    expect(page).to have_content("Network: HBO")
    # if no ending year is specified, should indicate show is still running
    expect(page).to have_content("2011 - Present")
    # synopsis not provided, should not be present
